...
// Check if payment method is ICICI and redirect to payment gateway
if($payment === 'icici') {
    // First insert the order with pending status
    $oreders_sql="INSERT INTO `orders`(`user_id`, `sub_total`, `shipping_charg`, `payment_method`, `total`, `status`) 
                     VALUES ('$user_id','$Sub_total','$Shipping_Cost','$payment','$total','pending')";
    if(mysqli_query($con,$oreders_sql)){
        $order_id=mysqli_insert_id($con);
        
        // Insert cart items to order details
        ...
        // Now redirect to ICICI payment gateway
        echo IciciPayment::initiatePayment($order_id, $total, $email, $mobile);
